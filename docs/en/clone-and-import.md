...
`git clone https://github.com/ByteLegendQuest/java-object-equals`

A project directory named `java-object-equals` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-object-equals` directory which is created just now.
